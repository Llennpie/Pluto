...
ifeq ($(shell git rev-parse --abbrev-ref HEAD),nightly)
    ...
    DEFINES += NIGHTLY GIT_HASH="\"$(shell $(GIT_HASH))\"" COMPILE_TIME="\"$(shell $(COMPILE_TIME))\""
endif
...
SRC_DIRS := src src/engine src/game src/audio src/bass_audio src/menu src/buffers src/saturn src/saturn/asset_extractor src/saturn/ui src/saturn/libs src/saturn/libs/cjson src/saturn/libs/imgui actors levels bin data assets asm lib sound
...
C_FILES           := $(foreach dir,$(SRC_DIRS),$(wildcard $(dir)/*.c)) $(LEVEL_C_FILES) actors/mario/mario_externs.c
...
$(BUILD_DIR)/Panimotion.mp3: sound/Panimotion.mp3
	$(call print,Copying:,$<,$@)
	$(V)cp $< $@
...
ifeq ($(TARGET_N64),1)
  ...
  $(EXE): $(O_FILES) $(MIO0_FILES:.mio0=.o) $(ULTRA_O_FILES) $(GODDARD_O_FILES) $(BUILD_DIR)/$(RPC_LIBS) $(BUILD_DIR)/$(DISCORD_SDK_LIBS) $(BUILD_DIR)/$(BASS_LIBS) $(BUILD_DIR)/$(COOPNET_LIBS) $(BUILD_DIR)/$(LANG_DIR) $(BUILD_DIR)/$(MOD_DIR) $(BUILD_DIR)/Panimotion.mp3 $(BUILD_DIR)/dynos
