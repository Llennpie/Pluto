...
void saturn_action_idle(struct MarioState *m) {
    ...
    if (!(enable_custom_anim && override_anim)) force_set_character_animation(m, (override_anim) ? (CharacterAnimID)selected_anim_index : idle_anim);
    if (m->marioObj == NULL) return;

    struct Animation *targetAnim = m->marioObj->header.gfx.animInfo.curAnim;
    // Animation-specific looping
    // This is all done with existing flags
    if (override_anim) {
    ...
    if (override_anim) {
        if (pause_anim) {
            set_anim_to_frame(m, paused_frame);
            ...
            if (is_anim_at_end(m)) {
                // Hanging at the last frame
                if (hang_anim) {
                    pause_anim = true;
                    paused_frame = m->marioObj->header.gfx.animInfo.animFrame;
                }
