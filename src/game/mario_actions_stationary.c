...
s32 check_common_idle_cancels(struct MarioState *m) {
    ...
    !(freeze_camera && gPlayer1Controller->buttonDown & R_TRIG)) {
    ...
    if ((is_editing_panim && enable_custom_anim) || override_anim ||
    (freeze_camera && gPlayer1Controller->buttonDown & R_TRIG)) { return 0; }
...
s32 check_common_hold_idle_cancels(struct MarioState *m) {
    ...
    if ((is_editing_panim && enable_custom_anim) || override_anim) { return 0; }
...
s32 act_idle(struct MarioState *m) {
    ...
    if (m->actionState == 3) {
        if (m->area && ((m->area->terrainType & TERRAIN_MASK) == TERRAIN_SNOW)) {
            ...
            if (!gDjuiInMainMenu && !freeze_camera) {
...
s32 act_first_person(struct MarioState *m) {
    ...
    if (m->actionState == 0) {
        ...
        m->actionState = 1;
    } else if ((!(m->input & INPUT_FIRST_PERSON) && !freeze_camera) || sp1C || gOverrideFreezeCamera || gExitCUpRequested) {
        gExitCUpRequested = FALSE;
        if (m->playerIndex == 0) {
            raise_background_noise(2);
